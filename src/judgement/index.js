...
(async () =>
{
    ...
    ctx.beginPath();
    ctx.rect(0, 0, pointSize, pointSize);
...
{
    if (note.isFake) return; // 忽略�?Note
    if (note.isScored && note.isScoreAnimated) return; // 已记分忽�?    if (note.time - this.judgeTimes.bad > currentTime) return; // 不在记分范围内忽�?    
    if (!note.isScored)
    {
